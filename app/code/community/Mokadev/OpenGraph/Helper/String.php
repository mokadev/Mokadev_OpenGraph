...
class Mokadev_OpenGraph_Helper_String extends Varien_Object
{
    ...
    public function getHtmlSubstr($length)
    {
        ...
        for ($i = 0; $i <= mb_strlen($this->_input, $this->_enc); $i++) {
            ...
            if ($this->_symbolsCount >= $length || $i >= mb_strlen(@$this->_input, $this->_enc) - 1) {
                //we don't cut a word
                if ($char != " " && $this->_symbolsCount >= $length){
                    continue;
                }
                $realPosition = $i;
                $closingTags = $this->_getClosingTags();
                $content = mb_substr(@$this->_input, 0, ($realPosition + 1), $this->_enc) . $closingTags . ($this->_symbolsCount >= $length ? $this->readMore : null);
                break;
            }
        }
